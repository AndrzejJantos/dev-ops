...
# Function: Setup cron job for cleanup
setup_cleanup_cron() {
    log_info "Setting up daily cleanup cron job..."
...
EOF
    ...
    local cron_entry="0 2 * * * ${cleanup_script} >> ${LOG_DIR}/cleanup.log 2>&1"

    # Check if cron entry already exists
    if crontab -u "$DEPLOY_USER" -l 2>/dev/null | grep -qF "$cleanup_script"; then
        log_info "Cleanup cron job already exists"
    else
        # Add cron job
        (crontab -u "$DEPLOY_USER" -l 2>/dev/null; echo "$cron_entry") | crontab -u "$DEPLOY_USER" -
